...
def focus_engine():
    ...
    # We define every possible place this file could be sitting
    options = [
        os.path.join(_script_dir, "focus_rf_model.pkl"),           # Same folder as dmb.py
        os.path.join(_script_dir, "Backend", "focus_rf_model.pkl") # Subfolder named Backend
    ]

    clf = None
    print(f"[DEBUG] System path: {_script_dir}", flush=True)

    for path in options:
        if os.path.exists(path):
            try:
                clf = joblib.load(path)
                print(f"[MODEL] Success! Loaded from: {path}", flush=True)
                break
            except Exception as e:
                print(f"[MODEL] Found file at {path} but load failed: {e}")

    if clf is None:
        # One last attempt: Check if we are accidentally in the subfolder already
        print("[MODEL] Not found in standard paths, checking local directory...", flush=True)
        if os.path.exists("focus_rf_model.pkl"):
            clf = joblib.load("focus_rf_model.pkl")
            print("[MODEL] Success! Loaded from local working directory.")
        else:
            print("[MODEL] focus_rf_model.pkl NOT FOUND. Rule-based only.", flush=True)
